...
*/
#ifndef _BDS_WORLD_BDS_
#define _BDS_WORLD_BDS_
...
class world
{
  private:
    ...
    inline auto sound_default_call()
    {
    ...
    inline auto explode_call(const D &d, const S &s)
    {
        ...
        return [this, d, s](const min::vec3<float> &p,
            ...
            // Explode the block
            this->explode(p, dir, scale, atlas, _explode_size, f, d, s);
        };
    }
    inline auto explode_default_call()
    {
    ...
    inline void drone_damage(const size_t drone_index, const min::tri<unsigned> &scale, const min::vec3<float> &dir, const float size, const float damage)
    {
        ...
        else
        {
            ...
            explode(p, flip, scale, atlas, size, f, dmg_drone_call(), sound_ex_call());
                                ...
                                const min::tri<unsigned> &scale, const float size, const bool is_charge, const R &ray_call)
    {
        ...
        case target_id::BLOCK:
        {
            ...
            explode_ray_block(t.get_position(), atlas, scale, size, ray_call);
  ...
  public:
    ...
    inline void add_block()
    {
        ...
        else
        {
            ...
            _grid.set_geometry(_preview, _scale, _preview_offset, _atlas_id, f);
        }
    }
    ...
    inline size_t scatter_ray(const min::tri<unsigned> &scale, const float size, const R &ray_call)
    {
        ...
        for (size_t i = 0; i < 4; i++)
        {
            ...
            if (explode_ray(r, t, scale, size, false, ray_call) != block_id::EMPTY)
            {
                count++;
            }
        }
